<%@ Page Title="" Language="C#" MasterPageFile="List.master" AutoEventWireup="true" %>

<asp:Content ID="Content1" ContentPlaceHolderID="ListsAndJournalHeaderContent" runat="Server">
    <title>Top 15 Movies of 2014</title>
    <meta name="description" content="List of best 15 movies in 2014" />
</asp:Content>

<asp:Content ID="Content2" ContentPlaceHolderID="ListsAndJournalBodyContent" runat="Server">
    <h1>Top 15 movies of 2014</h1>
    <ul class="List">
        <li>
            <h2>A MOST VIOLENT YEAR</h2>
            <h3>J.C.CHANDOR</h3>
        </li>
        <li>
            <h2>BIRDMAN</h2>
            <h3>ALEJANDRO GONZALEZ INARRITU</h3>
        </li>
        <li>
            <h2>BOYHOOD</h2>
            <h3>RICHARD LINKLATER</h3>
        </li>
        

        <li>
            <h2>GRAND BUDAPEST HOTEL, THE</h2>
            <h3>WES ANDERSON</h3>
        </li>
           <li>
            <h2>GONE GIRL</h2>
            <h3>DAVID FINCHER</h3>
        </li>
        <li>
            <h2>IDA</h2>
            <h3>PAWEL PAWLIKOWSKI</h3>
        </li>
        
         <li>
            <h2>LEVIATHAN</h2>
            <h3>ANDREY ZVYAGINTSEV</h3>
        </li>
             <li>
            <h2>LOCKE</h2>
            <h3>STEVEN KNIGHT</h3>
        </li>
                
        <li>
            <h2>NIGHTCRAWLER</h2>
            <h3>DON GILROY</h3>
        </li>
        
        <li>
            <h2>NYMPHOMANIAC VOL I & II</h2>
            <h3>LARS VON TRIER</h3>
        </li>
        <li>
            <h2>SNOWPIERCER</h2>
            <h3>BONG JOON-HO</h3>
        </li>
         <li>
            <h2>THE TALE OF PRINCESS KAGUYA</h2>
            <h3>ISAO TAKAHATA</h3>
        </li>
           <li>
            <h2>TWO DAYS ONE NIGHT</h2>
            <h3>JEAN-PIERRE DARDENNE, LUC DARDANNE</h3>
        </li>
                <li>
            <h2>UNDER THE SKIN</h2>
            <h3>JONATHAN GLAZER</h3>
        </li>
      
                <li>
            <h2>WINTER SLEEP</h2>
            <h3>NURI BILGE CEYLAN</h3>
        </li>

      
    </ul>
    <h4>Honourable Mentions</h4>
    <ul>
        <li>
            <h6>AMERICAN SNIPER</h6>
        </li>
                 <li>
            <h6>CHEF</h6>
        </li>
         <li>
            <h6>COLD IN JULY</h6>
        </li>
        
    
 <li>
            <h6>DAWN OF THE PLANET OF THE APES</h6>
        </li>
        

        <li>
            <h6>FURY</h6>
        </li>
        <li>
            <h6>GOODBYE TO LANGUAGE</h6>
        </li>
        <li>
            <h6>IMITATION GAME, THE</h6>
        </li>
        <li>
            <h6>INTERSTELLAR</h6>
        </li>
               <li>
            <h6>JIGARTHANDA</h6>
        </li>
                <li>
            <h6>KAJAKI</h6>
        </li>
          <li>
            <h6>PRIDE</h6>
        </li>
          <li>
            <h6>SELMA</h6>
        </li>

         <li>
            <h6>THE THEORY OF EVERYTHING</h6>
        </li>

   <li>
            <h6>WHIPLASH</h6>
        </li>

    </ul>
    <h4>Notable Mentions</h4>
    <ul>        
          <li>
            <h6>BIG HERO 6</h6>
        </li>
        
        
        <li>
            <h6>EDGE OF TOMORROW</h6>
        </li>
        
         <li>
            <h6>GODZILLA</h6>
        </li>
                <li>
            <h6>GUARDIANS OF THE GALAXY</h6>
        </li>
           <li>
            <h6>HOW TO TRAIN YOUR DRAGON 2</h6>
        </li>
                 <li>
            <h6>MR. PEABODY AND SHERMAN</h6>
        </li>
        <li>
            <h6>NOAH</h6>
        </li>
        <li>
            <h6>PADDINGTON</h6>
        </li>
        <li>
            <h6>THE LEGO MOVIE</h6>
        </li>
     
         <li>
            <h6>THE BOXTROLLS</h6>
        </li>
          <li>
            <h6>THE LUNCHBOX</h6>
        </li>

         <li>
            <h6>X MEN - DAYS OF THE FUTURE PAST</h6>
        </li>
        
    </ul>
</asp:Content>
